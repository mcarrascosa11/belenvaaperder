...
export default function Home() {
  ...
  async function load() {
    ...
    } catch { setError('No se ha podido cargar el marcador.'); }
  ...
  async function save() {
    ...
    if (!n || n > 25000) return;
    const next = { ...scores, [player]: scores[player].map((x, i) => i === day - 1 ? { score: n, note: note.trim() || undefined } : x) };
    setScores(next);
    ...
    try {
      ...
      setScore(''); setNote('');
    } catch { setError('No se ha podido guardar.'); await load(); }
    finally { setSaving(false); }
  }
  async function clearAll() {
    if (!confirm('¿Borrar todas las puntuaciones?')) return;
    setSaving(true); setError('');
    try { const r=await fetch('/api/state',{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify(empty)}); if(!r.ok) throw new Error('reset'); setScores(empty); }
    catch { setError('No se ha podido reiniciar.'); }
    finally { setSaving(false); }
  }
  ...
  if (loading) return <main><div className="wrap"><p className="loading">Cargando marcador…</p></div></main>;
  return <main>
    <div className="wrap">
      ...
      <section className="card">
        ...
        <button className="primary" disabled={saving} onClick={save}>{saving ? 'Guardando…' : 'Guardar puntuación'}</button>
      </section>

      <section className="card tablecard">
        <div className="tablehead"><h2>Marcador</h2><button className="danger" onClick={clearAll} disabled={saving}>Reiniciar</button></div>
    ...
    <style jsx global>{`*{box-sizing:border-box}body{margin:0;background:#0b0b0d;color:#f4f1ea;font-family:Inter,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}button,input,select{font:inherit}button{cursor:pointer}.wrap{max-width:980px;margin:auto;padding:32px 20px 50px}.loading{color:#777}header{display:flex;justify-content:space-between;gap:20px;align-items:flex-end;margin-bottom:28px}.eyebrow{font-size:12px;letter-spacing:.16em;color:#a9a29a}h1{font-size:clamp(34px,7vw,72px);line-height:.92;margin:8px 0;font-weight:950;letter-spacing:-.06em}header p{margin:0;color:#aaa39a}.ghost,.danger{background:none;border:1px solid #39393e;color:#ddd;padding:10px 14px;border-radius:10px}.hero{display:grid;grid-template-columns:1fr auto 1fr;gap:14px;align-items:center;margin:25px 0}.scorebox{background:#151518;border:1px solid #29292f;border-radius:18px;padding:26px;text-align:center}.scorebox span{font-size:12px;letter-spacing:.16em;color:#aaa}.scorebox strong{display:block;font-size:clamp(36px,7vw,64px);letter-spacing:-.05em;margin:8px 0}.scorebox small{color:#777}.vs{font-weight:900;color:#777}.status,.loser,.error{border-radius:14px;padding:14px;text-align:center;margin-bottom:18px;font-weight:800;letter-spacing:.08em}.status{background:#141416;color:#c9c2ba;border:1px solid #29292f}.loser{background:#f3e8d0;color:#0d0d0e}.error{background:#2a1717;color:#f0b9b9;border:1px solid #5a2929}.card{background:#151518;border:1px solid #29292f;border-radius:18px;padding:22px;margin-top:16px}.card h2{margin:0 0 18px;font-size:20px}.formgrid{display:grid;grid-template-columns:repeat(2,1fr);gap:14px}label{font-size:12px;color:#aaa;display:grid;gap:7px}input,select{width:100%;background:#0e0e10;color:#f4f1ea;border:1px solid #38383e;border-radius:10px;padding:12px}.primary{margin-top:16px;width:100%;border:0;background:#f4f1ea;color:#0d0d0e;font-weight:900;border-radius:11px;padding:13px}.primary:disabled,.danger:disabled{opacity:.5;cursor:default}.muted{color:#666}.tablehead{display:flex;justify-content:space-between;align-items:center}.danger{color:#cf9d9d}table{width:100%;border-collapse:collapse;font-size:14px}th,td{padding:12px 8px;border-bottom:1px solid #29292f;text-align:right}th:first-child,td:first-child{text-align:left}th{color:#777;font-weight:600}.total td{border-bottom:0;padding-top:16px;font-weight:900}.total td:nth-child(2),.total td:nth-child(3){font-size:18px}footer{text-align:center;color:#626067;font-size:12px;margin-top:20px}@media(max-width:700px){header{align-items:flex-start;flex-direction:column}.hero{grid-template-columns:1fr 26px 1fr}.scorebox{padding:18px 10px}.formgrid{grid-template-columns:1fr}th,td{padding:10px 5px}}`}</style>
  </main>
}